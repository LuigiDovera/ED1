...
void bstPostOrderNode(TNode *t, void (*visit)(TNode *))// opera a visit() no node em vez de no data
{
  if(t != NULL)
  {
    bstPostOrderNode(t->left, visit);
    bstPostOrderNode(t->right, visit);
    visit(t);
  }
}// bstPostOrderNode
...
void *bstPurge(TNode *t)
{
  if(t != NULL)
  {
    bstPostOrderNode(t, deleteNode);
  }
  return NULL;
}